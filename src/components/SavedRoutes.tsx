import { MapPinPlusInside, MapPin, Clock, Navigation, Trash2 } from "lucide-react";
...
export default function SavedRoutes({ routes, onDelete }: Readonly<SavedRoutesProps>) {
  return (
    <>
      {/* Saved Routes */}
      <div className="mb-4">
        <h2 className="text-2xl font-bold text-foreground mb-4" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
          Saved Routes
        </h2>
      </div>
      ...
      {routes.length === 0 ? (
        <Card>
          <CardBody className="py-8 text-center">
            ...
            <p className="text-default-500">No routes saved yet. Start tracking to record your first route!</p>
          </CardBody>
        </Card>
      ) : (
              ...
              <CardBody className="gap-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium">{route.label || "Untitled Route"}</span>
                  </div>
                  <div className="flex items-center gap-2">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="flex flex-col">
                    ...
                    <span className="text-lg font-semibold">{formatDistance(route.totalDistance)}</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-sm text-default-500">Duration</span>
                    <span className="text-lg font-semibold">{formatDuration(route.startTime, route.finishTime)}</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-sm text-default-500">GPS Points</span>
                    <span className="text-lg font-semibold">{route.positions.length}</span>
                  </div>
                </div>
