...
export default function Routes() {
  ...
  const { routes: savedRoutes, addRoute, removeRoute, isAuthorized } = useRouteStorage();
          ...
          <CardBody className="gap-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-success">{gps.statusMessage}</span>
              </div>
              {wakeLock.isActive && (
                <span className="text-xs text-default-500 bg-default-100 px-2 py-1 rounded">Screen stays awake</span>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div className="flex flex-col">
                ...
                <span className="text-lg font-semibold">{formatDuration(routeStartTime, null)}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-sm text-default-500">Distance</span>
                <span className="text-lg font-semibold">{formatDistance(gps.totalDistanceMeters)}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-sm text-default-500">GPS Points</span>
                <span className="text-lg font-semibold">{gps.trackPoints.length}</span>
              </div>
              {gps.currentAccuracy !== null && (
                <div className="flex flex-col">
                  <span className="text-sm text-default-500">Accuracy</span>
                  <span className="text-lg font-semibold">±{Math.round(gps.currentAccuracy)}m</span>
                </div>
              )}
            </div>
