...
export function useGpsTracker(options: UseGpsTrackerOptions = {}) {
   ...
   */
  const getStabilizedPoint = useCallback((buffer: GeoPoint[]): GeoPoint => {
    // Weight by inverse of accuracy (lower accuracy = higher weight)
    const weights = buffer.map((p) => 1 / p.accuracy);
    const totalWeight = weights.reduce((sum, w) => sum + w, 0);

    const lat = buffer.reduce((sum, p, i) => sum + p.latitude * weights[i], 0) / totalWeight;
    const lng = buffer.reduce((sum, p, i) => sum + p.longitude * weights[i], 0) / totalWeight;
    ...
    const avgAltitude =
      pointsWithAltitude.length > 0
        ? pointsWithAltitude.reduce((sum, p) => sum + (p.altitude ?? 0), 0) / pointsWithAltitude.length
        : null;

    // Use the last valid heading/speed from the buffer
    const lastValidSpeed = [...buffer].reverse().find((p) => p.speed !== null)?.speed ?? null;
    const lastValidHeading = [...buffer].reverse().find((p) => p.heading !== null)?.heading ?? null;
   ...
   */
  const getHeadingDelta = useCallback((heading1: number, heading2: number): number => {
   ...
   */
  const getDistance3D = useCallback((point1: GeoPoint, point2: GeoPoint): number => {
    ...
    const handlePosition = (position: GeolocationPosition) => {
      ...
      if (isStabilizingForEndRef.current) {
        if (point.accuracy <= maxAccuracyThreshold) {
          ...
          if (stabilizationBufferRef.current.length >= stabilizationReadings) {
            const endPoint = getStabilizedPoint(stabilizationBufferRef.current);
            ...
            if (lastRegisteredPointRef.current) {
              const lastSegment = getDistance3D(lastRegisteredPointRef.current, endPoint);
      ...
      if (!startPointRef.current) {
        if (point.accuracy <= maxAccuracyThreshold) {
          ...
          if (stabilizationBufferRef.current.length >= stabilizationReadings) {
            const startPoint = getStabilizedPoint(stabilizationBufferRef.current);
